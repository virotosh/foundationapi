...
eda64 = signal.resample_poly(eda4, up=64, down=64)*1.5
print(len(eda64))
print(eda64[:4])
...
# init model
ckpt_path = './data/epoch=199-step=3400.ckpt' # stress
#ckpt_path = './data/epoch=199-step=1400.ckpt' #mw
#ckpt_path = './data/epoch=199-step=5800-v9.ckpt'
#ckpt_path = './data/epoch=199-step=6600.ckpt'
model = LitSensorPT.load_from_checkpoint(ckpt_path, map_location=torch.device("cpu"))
model.eval()
stress_prob = []
for idx in range(len(eda64)-512):
    # if idx < 60000:
    #     continue
    padding = [0.0 for i in range(512)]
#    params["empatica"][0][0] =  df['hr'].tolist()[idx:idx+512]
#    params["empatica"][0][1] =  eda64[idx:idx+512]
    ...
    try:
        ...
        res = dict(zip(["stress"], probs[1:2]))
        print(res, eda64[idx])
        stress_prob.append(probs[1])
    except Exception as e:
        print('%s', repr(e))
        break

    if (idx >= len(eda64)-512):
        break
print(len(stress_prob))
df = df.drop(df.index[:512])
df['mental_workload_prob'] = stress_prob
df.to_csv('stresstest5.csv', encoding='utf-8', index=False, header=True)
